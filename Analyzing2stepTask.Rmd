...
if (preprocess_data) {
  ...
  write.csv(class_lagged_effects, file.path(data_dir, paste("class_lagged_effects_", data_source, ".csv")), row.names = F)
  write.csv(class_smooth_lagged_effects, file.path(data_dir, paste("class_smooth_lagged_effects_", data_source, ".csv")), row.names = F)
  write.csv(lagged_switch_stay, file.path(data_dir, paste("lagged_switch_stay_", data_source, ".csv")), row.names = F)
  write.csv(lagged_switch_stay_smooth, file.path(data_dir, paste("lagged_switch_stay_smooth_", data_source, ".csv")), row.names = F)
  write.csv(lagged_switch_stay_stage2, file.path(data_dir, paste("lagged_switch_stay_stage2_", data_source, ".csv")), row.names = F)
} else {
  class_lagged_effects         = read.csv(file.path(data_dir, paste("class_lagged_effects_", data_source, ".csv")))
  class_smooth_lagged_effects  = read.csv(file.path(data_dir, paste("class_smooth_lagged_effects_", data_source, ".csv")))
  lagged_switch_stay           = file.path(data_dir, paste("lagged_switch_stay_", data_source, ".csv"))
  lagged_switch_stay_smooth    = file.path(data_dir, paste("lagged_switch_stay_smooth_", data_source, ".csv"))
  lagged_switch_stay_stage2    = read.csv(file.path(data_dir, paste("lagged_switch_stay_stage2_", data_source, ".csv")))
}
...
hyb_nok_mod = lmer(BIC_zis ~ params + (params | SubjID), data = lmer_dat1)
summary(hyb_nok_mod)
```
