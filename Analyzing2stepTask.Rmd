...
if (preprocess_data) {
    ...
    for (fileid in (1:dim(mat_file)[3])) {
      ...
      for (hier in 1:2) {
        ...
        for (run in 1:4) {
          ...
          subj_par4 = c(subjID, run, par4[1], NA, par4[2], NA, 0, par4[4], par4[3], NA, NA, BIC, AIC, hier, num_par)
          # Add to ts_params
  ...
  ts_params$params = paste("klaus", ifelse(ts_params$params == 1, "hier", "flat"), ts_params$num_par, sep = "_")
...
gg_b_flat_hier = ggplot(b_molten, aes(b1.klaus_flat_4, b1.klaus_hier_4)) +
  geom_point() +
  geom_smooth(method = "lm")
gg_b_flat_hier

gg_b_k_m = ggplot(b_molten, aes(b1.klaus_flat_4, b1.l0)) +
  geom_point() +
  geom_smooth(method = "lm")
gg_b_k_m
```
...
ggplot(subset(params_regr_dat, params %in% c("klaus_flat_4", "klaus_hier_4", "l0") & odds < 5 & odds > -5), aes(w, odds)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_grid(~ params)
```
...
selected_parameter_dat = subset(ts_params_molten, params == "klaus_hier_4" & parameter %in% c("a1", "a2", "b1", "b2", "p", "k", "w"), select = c("SubjID", "run", "training", "training_s1", "parameter", "estimate"))   # params == "l1"
...
# Prepare data
ts_plot_dat = subset(ts_params, grepl("klaus", params))
