...
if (preprocess_data) {
  # Read in all parameter estiamtes
  ...
  colnames(all_genrec) = c("SubjID", "model", "hier", "n_par", "a1", "a2", "b1", "b2", "l", "w", "p", "k", "run", "NLL", "BIC", "AIC")
...
a2_dat = subset(all_genrec, select = c("SubjID", "run", "model", "hier", "n_par", "a2"))
a2_dat = reshape(a2_dat, timevar = "model", direction = "wide", idvar = c("SubjID", "run", "hier", "n_par"))

p_dat = subset(all_genrec, select = c("SubjID", "run", "model", "hier", "n_par", "p"))
p_dat = reshape(p_dat, timevar = "model", direction = "wide", idvar = c("SubjID", "run", "hier", "n_par"))

w_dat = subset(all_genrec, select = c("SubjID", "run", "model", "hier", "n_par", "w"))
w_dat = reshape(w_dat, timevar = "model", direction = "wide", idvar = c("SubjID", "run", "hier", "n_par"))

wide_dat = merge(a2_dat, p_dat)
wide_dat = merge(wide_dat, w_dat)

ggplot(wide_dat, aes(a2.M, a2.K, color = hier)) +
  geom_point(alpha = .2) +
  labs(x = "alpha Maria", y = "alpha Klaus") +
  facet_grid(~ n_par)

ggplot(wide_dat, aes(p.M, p.K)) +
  geom_point() +
  labs(x = "p Maria", y = "p Klaus") +
  facet_grid(~ n_par)

ggplot(wide_dat, aes(w.M, w.K)) +
  geom_point() +
  labs(x = "w Maria", y = "w Klaus") +
  facet_grid(~ n_par)

hier_dat = subset(all_genrec, select = c("SubjID", "run", "model", "hier", "n_par", "a2"))
hier_dat = reshape(hier_dat, timevar = "hier", direction = "wide", idvar = c("SubjID", "run", "model", "n_par"))

ggplot(hier_dat, aes(a2.1, a2.0, color = model)) +
  geom_point() +
  labs(x = "hier", y = "flat") +
...
ggplot(hier_dat, aes(w.1, w.0, color = model)) +
  geom_point() +
  labs(x = "hier", y = "flat") +
  facet_grid(~ n_par)
```
...
ggplot(subset(all_genrec), aes(model, BIC, fill = n_par)) +
...
K = reshape(subset(all_genrec, hier == 0 & model == "K", select = c("SubjID", "n_par", "run", "BIC", "model")),
            idvar = c("SubjID", "run", "model"), timevar = "n_par", direction = "wide")
M = reshape(subset(all_genrec, hier == 0 & model == "M", select = c("SubjID", "n_par", "run", "BIC", "model")),
            idvar = c("SubjID", "run", "model"), timevar = "n_par", direction = "wide")
rel_BIC = rbind(K, M)
rel_BIC$r4 = with(rel_BIC, BIC.7 - BIC.4)
rel_BIC$r6 = with(rel_BIC, BIC.7 - BIC.6)
rel_BIC_molten = melt(subset(rel_BIC, select = c("SubjID", "run", "model", "r4", "r6")),
                      id.vars = c("SubjID", "run", "model"))

ggplot(subset(rel_BIC_molten), aes(variable, value, color = model)) +
...
for (h in levels(factor(all_genrec$hier))) {
  gg_w = ggplot(subset(all_genrec, hier == h), aes(run, w, color = model)) +
    stat_summary(fun.data = mean_cl_normal, geom = "pointrange") +
    geom_point(alpha = .2, position = "jitter") +
    facet_grid(training_s1 ~ n_par)
  plot(gg_w)
}
```
    ...
    for (fileid in (1:dim(mat_file)[3])) {
      ...
      for (hier in 1:2) {
        ...
        for (run in 1:4) {
          ...
          subj_par4 = c(subjID, run, par4[1], NA, NA, NA, 0, par4[num_par], par4[num_par-1], NA, NA, BIC, AIC, hier, num_par)   # I'm just taking a1, p, and w for now because the orders of the other parameters differ by models
          
          # Add to ts_params
          ts_params = rbind(ts_params, subj_par4)
        }
      }
    }
  }
  colnames(ts_params) = c(ts_colnames, "params", "num_par")
  ts_params$params = paste("klaus", ifelse(ts_params$params == 1, "flat", "hier"), ts_params$num_par, sep = "_")
...
for (file_name in file_names) {
  ...
  num_par = 0
  par_file = cbind(par_file, params, num_par, session)   # can't count how many parameters because some are 0, some 1, some 0.5... would have to check for parameters with sd == 0 to make it work
...
params_regr_dat = merge(subset(logist_regr_data_with_rep, effect == "int", select = -AIC), ts_params)

ggplot(subset(params_regr_dat, params %in% c("klaus_flat_4", "klaus_hier_4", "klaus_flat_6", "klaus_hier_6", "l0", "hyb") & odds < 5 & odds > -5), aes(w, odds)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_grid( ~ params)
```
...
c_vs_train = c(-2, 1, 1)
mb_vs_mf = c(0, -1, 1)
contrasts(mod_dat$training_s1) = cbind(mf_vs_rest, co_vs_mb)
...
# Post-hoc t.tests
mod_dat_long = reshape(mod_dat, timevar = "run", idvar = c("SubjID", "training", "training_s1"), direction = "wide")
mod_dat_long$dw12 = with(mod_dat_long, `estimate.Run 2` - `estimate.Run 1`)
t.test(subset(mod_dat_long, training_s1 == "co")$dw12)
t.test(subset(mod_dat_long, training_s1 == "mb")$dw12)
t.test(subset(mod_dat_long, training_s1 == "mf")$dw12)
```
...
ts_plot_dat = subset(ts_params, params %in% c("hyb", "l0", "l1", "klaus_hier_4", "klaus_flat_4"),
                     select = -c(NLL, BIC, AIC, num_par, a1, a2, b1, b2, l, p, k, n_trials))
