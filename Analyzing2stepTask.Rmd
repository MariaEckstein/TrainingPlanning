...
if (preprocess_data) {
  ...
  for (runi in unique(stay_dat$run)) {
    for (traini in unique(stay_dat$training_s1)) {
      ...
      if (!empty(model_dat)) {
        ...
        # Small model: Just reward * transition
        ...
        switch_stay_stats_run_train[row:(row+3),2] = runi   # Run
        switch_stay_stats_run_train[row:(row+3),3] = traini   # Group
        switch_stay_stats_run_train[row:(row+3),4:7] = coefficients(summary(mod))   # beta, se, z, and p values
        switch_stay_stats_run_train[row:(row+3),8] = summary(mod)$AICtab[1]   # AIC
        switch_stay_stats_run_train[row:(row+3),9] = summary(mod)$AICtab[2]   # BIC
        switch_stay_stats_run_train[row:(row+3),10] = "rew*tra"   # Name of the model
        
        row = row + 4
        
        # Full(er) model: Keyrep + reward * transition
  ...
  for (runi in unique(stay_dat$run)) {
    ...
    if (!empty(model_dat)) {
      ...
      # Small model: Just training * reward * transition
      ...
      switch_stay_stats_run[row:(row+7),2] = runi   # Run
      switch_stay_stats_run[row:(row+7),3:5] = aov   # z and p values
      switch_stay_stats_run[row:(row+7),6] = summary(mod)$AICtab[1]   # AIC
      switch_stay_stats_run[row:(row+7),7] = summary(mod)$AICtab[2]   # BIC
      switch_stay_stats_run[row:(row+7),8] = "tra*rew*gro"   # Model name

      row = row + 8
      
      # Full(er) model: Keyrep + training * reward * transition
      mod = glmer(stay_first_stage ~ repeat_first_key + reward_prev_trial * transi_prev_trial * training_s1 + (reward_prev_trial * transi_prev_trial | SubjID),   # Could also use update()?
            family = binomial, control = glmerControl(optimizer = "bobyqa"),
            data = model_dat)
      aov = Anova(mod, type = 3)
  ...
  # SMall model
  mod = glmer(stay_first_stage ~ reward_prev_trial + transi_prev_trial + reward_prev_trial:transi_prev_trial + reward_prev_trial:training_s1 + reward_prev_trial:transi_prev_trial:training_s1 + reward_prev_trial:run + reward_prev_trial:transi_prev_trial:run + reward_prev_trial:transi_prev_trial:training_s1:run +
      ...
      data = model_dat)
  switch_stay1 = cbind(coefficients(summary(mod)), "rew*tra*gro*run", summary(mod)$AICtab[1], summary(mod)$AICtab[2])
...
switch_stay_stats_run = read.csv(file.path(data_dir, paste("switch_stay_stats_run", data_source, ".csv", sep = "")))
...
### Check if models with key or without key have a better model fit
# Prepare BIC data
...
### Separate logistic regression for each participant (switch_stay ~ stay + reward + transition + interaction + correct + keyrep)
...
if (do_exclude_trials) {
  ...
  ts_params = merge(ts_params, subset(count_trials, select = -n_trials))
  ts_params = subset(ts_params, !few_trials)
  ts_params$few_trials = NULL
  
  # Exclude data sets with abnormal BICs
...
```{r, echo = F, warning = F, message = F, fig.height = 4.5, fig.width = 9, fig.cap = "Model fits (BIC) for different combinations of parameters. Lower BICs indicate *better* model fit."}
# Plot average BICs
...
rel_BIC$nokp = with(rel_BIC, BIC.nopk - BIC.hyb)
...
rel_BIC$nok_mb = with(rel_BIC, BIC.nok_mb - BIC.hyb)
rel_BIC$nok_mf = with(rel_BIC, BIC.nok_mf - BIC.hyb)
rel_BIC$nok_l0 = with(rel_BIC, BIC.nok_l0 - BIC.hyb)
rel_BIC$nok_l1 = with(rel_BIC, BIC.nok_l1 - BIC.hyb)
rel_BIC$a1b1_nok = with(rel_BIC, BIC.a1b1_nok - BIC.hyb)
summary(rel_BIC)
...
## nok_l0
nok_l0_total = sum(!is.na(rel_BIC$nok_l0))
nok_l0_better = sum(rel_BIC$nok_l0 > 0, na.rm = T)
binom.test(nok_l0_better, nok_l0_total)   # ***
## nok_l1
nok_l1_total = sum(!is.na(rel_BIC$nok_l1))
nok_l1_better = sum(rel_BIC$nok_l1 > 0, na.rm = T)
binom.test(nok_l1_better, nok_l1_total)   # ***
## nok_l1 > nok_l0
nok_l1_total = sum(!is.na(rel_BIC$nok_l0 - rel_BIC$nok_l1))
nok_l1_better = sum((rel_BIC$nok_l0 - rel_BIC$nok_l1) > 0, na.rm = T)
binom.test(nok_l1_better, nok_l1_total)   # * (p = 0.01635)
```
...
# Add training column
selected_parameter_dat = subset(ts_params_molten, params == "nok_l1" & parameter %in% c("a1", "a2", "b1", "b2", "p", "w"), select = c("SubjID", "run", "training", "training_s1", "parameter", "estimate"))
