...
if (preprocess_data) {
  ...
  all_pars = as.data.frame(readMat(file.path(model_dir, "all_pars.mat"))$all.pars)
  all_pars = subset(all_pars, select = -c(5:9, 18))
  colnames(all_pars) = c("SubjID", "model", "iteration", "n_par", "a1", "a2", "b1", "b2", "l", "w", "p", "k", "run", "NLL", "BIC", "AIC")
  all_pars$model = factor(all_pars$model, levels = c(103, 112), labels = c("M", "K"))
  write.csv(all_pars, file.path(model_dir, "all_pars.csv"), row.names = F)
}
all_pars$iteration = factor(all_pars$iteration)

a2_dat = subset(all_pars, select = c("SubjID", "run", "model", "iteration", "n_par", "a2"))
a2_dat = reshape(a2_dat, timevar = "model", direction = "wide", idvar = c("SubjID", "run", "iteration", "n_par"))

p_dat = subset(all_pars, select = c("SubjID", "run", "model", "iteration", "n_par", "p"))
p_dat = reshape(p_dat, timevar = "model", direction = "wide", idvar = c("SubjID", "run", "iteration", "n_par"))

w_dat = subset(all_pars, select = c("SubjID", "run", "model", "iteration", "n_par", "w"))
w_dat = reshape(w_dat, timevar = "model", direction = "wide", idvar = c("SubjID", "run", "iteration", "n_par"))

wide_dat = merge(a2_dat, p_dat)
wide_dat = merge(wide_dat, w_dat)

ggplot(wide_dat, aes(a2.M, a2.K, color = iteration)) +
  geom_point(alpha = .2) +
...
it_dat = subset(all_pars, select = c("SubjID", "run", "model", "iteration", "n_par", "a2"))
it_dat = reshape(it_dat, timevar = "iteration", direction = "wide", idvar = c("SubjID", "run", "model", "n_par"))

ggplot(it_dat, aes(a2.1, a2.2, color = model)) +
  geom_point() +
  labs(x = "Iteration 1", y = "Iteration 2") +
  facet_grid(~ n_par)

it_dat = subset(all_pars, select = c("SubjID", "run", "model", "iteration", "n_par", "w"))
it_dat = reshape(it_dat, timevar = "iteration", direction = "wide", idvar = c("SubjID", "run", "model", "n_par"))

ggplot(it_dat, aes(w.1, w.2, color = model)) +
  geom_point() +
  labs(x = "Iteration 1", y = "Iteration 2") +
  facet_grid(~ n_par)
