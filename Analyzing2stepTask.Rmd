...
data_source = "2016from_scratch"        # can be either 'empirical' (2015 data) or 'simulations' or 2016 (2016 data)
do_exclude_trials = T                   # Exclude trials based on perseverance, RTs, etc?
ggsave_figures = F                      # Save figures as pngs onto the computer?
preprocess_data = F
figure_dir = "C:/Users/maria/MEGAsync/GSN/Wunderlich Lab/TrainingPlanningProject/Figures"
data_dir = "C:/Users/maria/MEGAsync/GSN/Wunderlich Lab/TrainingPlanningProject/BerkeleyData/TwoStep"
...
library("gridExtra"); library("nlme"); library("plyr"); library("dplyr"); library("reshape"); library("R.matlab"); library("simpleboot"); library("car"); library("mlogit"); library("zoo")
...
if (data_source == "2016from_scratch" & preprocess_data) {
  ...
  for (filename in filenames) {
    ...
    subj_file = as.data.frame(readMat(paste(data_dir, filename, sep = "/"))$params[[5]][[1]])
  ...
  write.csv(all_files, paste(data_dir, "all_files.csv", sep = "/"), row.names = F)
}
all_files = read.csv(paste(data_dir, "all_files.csv", sep = "/"))
...
attach(excluded_subj)
excluded_subj = excluded_subj[order(session, run, SubjID),c(2,3,1,5)]
detach(excluded_subj)
...
# ts_sim_no_nas = subset(ts_sim, !is.na(reward_prev_trial))
# stay_dat_sim  = ddply(ts_sim_no_nas, .(SubjID, session, run, transi_prev_trial, reward_prev_trial, training), summarize,
#                   stays = sum(stay_first_stage, na.rm = T),
#                   trials = length(stay_first_stage),
#                   stay_prob = mean(stay_first_stage, na.rm = T))
```
...
```{r, echo = F, warning = F}
gg_choice_variation = ggplot(choice_variation, aes(SubjID, variation1)) +
  geom_point(aes(color = variation1 > (1 - limits.stage1_choice_variation) | variation1 < limits.stage1_choice_variation)) +
  scale_color_manual(values = c("black", "red")) +
  geom_hline(yintercept = (1 - limits.stage1_choice_variation), color = "red") +
  geom_hline(yintercept = limits.stage1_choice_variation, color = "red") +
...
```{r, echo = F, fig.height = 4, fig.width = 3.5, fig.cap = "What model-based and model-free strategies in the 2-step task should look like."}

sim_dat_mb = expand.grid(Strategy = "Model-based", Transition = c("Common", "Rare"), Reward = c("Reward", "No reward"))
sim_dat_mf = expand.grid(Strategy = "Model-free", Transition = c("Common", "Rare"),  Reward = c("Reward", "No reward"))
sim_dat_mf$stay_prob = c(.8, .8, .6, .6)
sim_dat_mb$stay_prob = c(.8, .6, .6, .8)
sim_dat = as.data.frame(rbind(sim_dat_mb, sim_dat_mf))

ggplot(sim_dat, aes(Reward, stay_prob, fill = Transition)) +
...
gg_overall_stats_s1
# 5 groups
gg_overall_stats = ggplot(switch_stay_stats, aes(predictor, chisq, shape = sig)) +
  geom_point(color = "darkblue") +
  scale_shape_manual(values = c(1, 16)) +
  geom_text(aes(label = ifelse(as.numeric(p) < 0.1, round(as.numeric(p), 2), "")), hjust = 0, vjust = 0, size = 3) +
  labs(x = "", y = "Chi squared") +
...
```{r, echo = F, warning = F, message = F, fig.cap = "Simulated log odds"}
log_odds_sim_dat = expand.grid(effect = c(rep("Stay", 10), rep("Reward", 10), rep("Transition", 10), rep("Interaction", 10)), Strategy = c("Model-based", "Model-free"), log_odds = NA)
log_odds_sim_dat$log_odds[log_odds_sim_dat$Strategy == "Model-based"] = c(seq(.4, .85, .05), seq(  0, .45, .05), seq(-.2, .25, .05), seq( .4, .85, .05))
log_odds_sim_dat$log_odds[log_odds_sim_dat$Strategy == "Model-free"]  = c(seq(.4, .85, .05), seq( .6,1.05, .05), seq(-.2, .25, .05), seq(-.2, .25, .05))

gg_logodds_pic = ggplot(log_odds_sim_dat, aes(effect, log_odds)) +
  stat_summary(fun.data = "mean_cl_normal", geom = "errorbar", color = "blue") +
  geom_point() +
...
source("TP_functions.R")
# ts_params_all     = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_aabblpkw.csv"), params = "ALL", num_par = 8)
# ts_params_all     = subset(ts_params_all, run == "Run 2")
# ts_params_1alpha  = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_abblpkw.csv"), params = "1a", num_par = 7)
# ts_params_1alpha  = subset(ts_params_1alpha, run == "Run 2")
# ts_params_1beta   = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_aablpkw.csv"), params = "1b", num_par = 7)
# ts_params_1beta   = subset(ts_params_1beta, run == "Run 2")
# ts_params_nolambda= cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_aabbpkw.csv"), params = "no l", num_par = 7)
# ts_params_nolambda= subset(ts_params_nolambda, run == "Run 2")
# ts_params_nop     = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_aabblkw.csv"), params = "no p", num_par = 7)
# ts_params_nop     = subset(ts_params_nop, run == "Run 2")
# ts_params_nok     = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_aabblpw.csv"), params = "no k", num_par = 7)
# ts_params_nok     = subset(ts_params_nok, run == "Run 2")
# ts_params_now     = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_aabblpk.csv"), params = "no w", num_par = 7)
# ts_params_now     = subset(ts_params_now, run == "Run 2")
# 
# ts_params_1a_1b     = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_ablpkw.csv"), params = "1a,1b", num_par = 6)
# ts_params_1a_1b     = subset(ts_params_1a_1b, run == "Run 2")
# ts_params_1a_1b_nol = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_abpkw.csv"), params = "a,b,p,k,w", num_par = 5)
# ts_params_1a_1b_nol = subset(ts_params_1a_1b_nol, run == "Run 2")
# ts_params_1a_1b_nop = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_ablkw.csv"), params = "a,b,l,k,w", num_par = 5)
# ts_params_1a_1b_nop = subset(ts_params_1a_1b_nop, run == "Run 2")
# ts_params_1a_1b_nok = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_ablpw.csv"), params = "a,b,l,p,w", num_par = 5)
# ts_params_1a_1b_nok = subset(ts_params_1a_1b_nok, run == "Run 2")
# ts_params_1a_1b_now = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_ablpk.csv"), params = "a,b,l,p,k", num_par = 5)
# ts_params_1a_1b_now = subset(ts_params_1a_1b_now, run == "Run 2")
# 
# ts_params_1a_1b_nol_nop = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_abkw.csv"), params = "a,b,k,w", num_par = 4)
# ts_params_1a_1b_nol_nop = subset(ts_params_1a_1b_nol_nop, run == "Run 2")
# ts_params_1a_1b_nol_nok = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_abpw.csv"), params = "a,b,p,w", num_par = 4)
# ts_params_1a_1b_nol_nok = subset(ts_params_1a_1b_nol_nok, run == "Run 2")
# ts_params_1a_1b_nol_now = cbind(read_in_2step_parameters3("k_and_p_can_be_negative/parameter_data_ga_fmincon_abpk.csv"), params = "a,b,p,k", num_par = 4)
# ts_params_1a_1b_nol_now = subset(ts_params_1a_1b_nol_now, run == "Run 2")

### Combine all data frames
# ts_params = rbind(ts_params_all,
#                   ts_params_nop, ts_params_nok, ts_params_now, ts_params_1alpha, ts_params_1beta, ts_params_nolambda,
#                   ts_params_1a_1b,
#                   ts_params_1a_1b_nop, ts_params_1a_1b_nok, ts_params_1a_1b_now, ts_params_1a_1b_nol,
#                   ts_params_1a_1b_nol_nop, ts_params_1a_1b_nol_nok, ts_params_1a_1b_nol_now)

### Read in data frames for all combinations of parameters
ts_colnames = c("a1", "a2", "b1", "b2", "l", "p", "k", "w", "BIC", "SubjID", "session", "run", "f")
ts_params_aabblpkw = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_aabblpkw.csv", col.names = ts_colnames, header = F),
                          params = "all", num_par = 8)
ts_params_aabbpkw = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_aabbpkw.csv", col.names = ts_colnames, header = F),
                          params = "nol", num_par = 7)
ts_params_aablpkw = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_aablpkw.csv", col.names = ts_colnames, header = F),
                          params = "1b", num_par = 7)
ts_params_aabblpw = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_aabblpw.csv", col.names = ts_colnames, header = F),
                          params = "nok", num_par = 7)
ts_params_aabblpk = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_aabblpk.csv", col.names = ts_colnames, header = F),
                          params = "now", num_par = 7)
ts_params_aabblkw = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_aabblkw.csv", col.names = ts_colnames, header = F),
                          params = "nop", num_par = 7)
ts_params_abblpkw = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_abblpkw.csv", col.names = ts_colnames, header = F),
                          params = "1a", num_par = 7)
ts_params_abblpkw = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_abblpkw.csv", col.names = ts_colnames, header = F),
                          params = "1a", num_par = 7)
ts_params_ablpkw = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_ablpkw.csv", col.names = ts_colnames, header = F),
                          params = "1a1b", num_par = 6)
ts_params_ablkw = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_ablkw.csv", col.names = ts_colnames, header = F),
                          params = "ablkw", num_par = 5)
ts_params_ablpw = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_ablpw.csv", col.names = ts_colnames, header = F),
                          params = "ablpw", num_par = 5)
ts_params_abpkw = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_abpkw.csv", col.names = ts_colnames, header = F),
                          params = "abpkw", num_par = 5)
ts_params_ablpk = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_ablpk.csv", col.names = ts_colnames, header = F),
                          params = "ablpk", num_par = 5)
ts_params_abpw = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_abpw.csv", col.names = ts_colnames, header = F),
                          params = "abpw", num_par = 4)
ts_params_abpk = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_abpk.csv", col.names = ts_colnames, header = F),
                          params = "abpk", num_par = 4)
ts_params_abp = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_abp.csv", col.names = ts_colnames, header = F),
                          params = "abp", num_par = 3)
ts_params_abk = cbind(read.csv("BerkeleyData/TwoStep/ga_fmincon_abk.csv", col.names = ts_colnames, header = F),
                          params = "abk", num_par = 3)

### Combine all data frames
ts_params = rbind(ts_params_aabblpkw,
                  ts_params_aabbpkw, ts_params_aablpkw, ts_params_abblpkw, ts_params_abblpkw, ts_params_aabblpk, ts_params_aabblpw, ts_params_aabblkw,
                  ts_params_ablpkw,
                  ts_params_ablpw, ts_params_abpkw, ts_params_ablpk, ts_params_ablkw,
                  ts_params_abpw, ts_params_abpk,
                  ts_params_abp, ts_params_abk)
# ts_params_$function.value = NULL

### Exclude bad subjects (more than 50% key repetition)
...
ts_params$b1 = ts_params$b1 / 20
ts_params$b2 = ts_params$b2 / 20
ts_params$p     = ts_params$p / 10
ts_params$k     = ts_params$k / 10
...
The models contained the parameters $\alpha 1$, $\alpha 2$, $\beta 1$, $\beta 2$, $\lambda$, $p$, $k$, and $w$ (table 1).

**Parameter**   | **Boundaries**  | **Significance**
----------------|-----------------|----------------
$\alpha 1$      | [0, 1]          | $1^\text{st}$-stage learning rate
$\alpha 2$      | [0, 1]          | $2^\text{nd}$-stage learning rate
$\beta 1$       | [0, 20]         | $1^\text{st}$-stage softmax temperature
$\beta 2$       | [0, 20]         | $2^\text{nd}$-stage softmax temperature
$\lambda$       | [0, 1]          | Discount of $1^\text{st}$-stage reward-based value update
$p$             | [-5, 5]         | Choice perseverance
$k$             | [-5, 5]         | Key perseverance
$w$             | [0.01, 0.99]    | Relative weight of model-based vs. model-free decision making

Parameters in all models were estimated using the same parameter boundaries (see *table 1* above). Models differed in the number of parameters estimated (leaving out $\alpha 2$, $\beta 2$, $\lambda$, $p$, $k$, or a combination of these).

The following parameter values were used when a parameter was not estimated:
- $\alpha 2$: Equals $\alpha 1$
- $\beta 2$: Equals $\beta 1$
- $\lambda$: 1
- $p$: 0
- $k$: 0
- $w$: 0.5
...
```{r, echo = F, fig.cap = "Simulated Parameter Data"}
par_sim = expand.grid(Parameter = c("alpha", "beta", "p", "k", "w"), Strategy = c("Model-based", "Model-free"))
par_sim$value = c(.3, .2, .08, .02, .8, .3, .2, .08, .02, .3)

gg_par_sim = ggplot(par_sim, aes(Parameter, value, fill = Strategy)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_point(position = position_dodge(width = .9)) +
  coord_cartesian(ylim = c(0, 1)) +
  scale_fill_manual(values = c("darkgreen", "sienna4")) +
  scale_y_continuous(breaks = 0, labels = 0) +
  theme(legend.position = "none")#c(.2, .7), legend.background = element_rect(fill = "transparent"))
...
# selected_parameter_dat$train_order = NA
# selected_parameter_dat$train_order[selected_parameter_dat$SubjID %in% seq(1,101,4) | selected_parameter_dat$SubjID %in% seq(3,103,4)] = "ArrTol"
# selected_parameter_dat$train_order[selected_parameter_dat$SubjID %in% seq(2,102,4) | selected_parameter_dat$SubjID %in% seq(4,104,4)] = "TolArr"
...
regression_param_dat = subset(selected_parameter_dat, parameter == "w" & run %in% c(1,2))
with(regression_param_dat, by(estimate, list(training_s1, run), mean))
co_vs_mbmf = c(-2, 1, 1)
mb_vs_mf = c(0, 1, -1)
contrasts(regression_param_dat$training_s1) = cbind(co_vs_mbmf, mf_vs_mb)
...
for (parami in unique(selected_parameter_dat$parameter)) {
  for (subj in unique(selected_parameter_dat$SubjID)) {
    ...
    estim1 = subset(selected_parameter_dat, parameter == parami & SubjID == subj & run == 1)$estimate
    estim2 = subset(selected_parameter_dat, parameter == parami & SubjID == subj & run == 2)$estimate
    estim3 = subset(selected_parameter_dat, parameter == parami & SubjID == subj & run == 3)$estimate
    estim4 = subset(selected_parameter_dat, parameter == parami & SubjID == subj & run == 4)$estimate
