...
if (ggsave_figures) {
  ...
  ggsave(plot = stay_switch_plot_s1, filename = paste(figure_dir, "stay_switch_plot_s1.png", sep = "/"), width = 5, height = 3.5)
}
```
...
if (preprocess_data) {
  ...
  write.csv(class_smooth_lagged_effects, file.path(regression_dir, paste("class_smooth_lagged_effects_", data_source, ".csv", sep = "")), row.names = F)
  write.csv(lagged_switch_stay, file.path(regression_dir, paste("lagged_switch_stay_", data_source, ".csv", sep = "")), row.names = F)
  write.csv(lagged_switch_stay_smooth, file.path(regression_dir, paste("lagged_switch_stay_smooth_", data_source, ".csv", sep = "")), row.names = F)
  ...
  write.csv(new_smooth_lagged_effects, file.path(regression_dir, paste("new_smooth_lagged_effects_", data_source, ".csv", sep = "")), row.names = F)
}
## Read in data
class_lagged_effects         = read.csv(file.path(regression_dir, paste("class_lagged_effects_", data_source, ".csv", sep = "")))
class_smooth_lagged_effects  = read.csv(file.path(regression_dir, paste("class_smooth_lagged_effects_", data_source, ".csv", sep = "")))
lagged_switch_stay           = read.csv(file.path(regression_dir, paste("lagged_switch_stay_", data_source, ".csv", sep = "")))
lagged_switch_stay_smooth    = read.csv(file.path(regression_dir, paste("lagged_switch_stay_smooth_", data_source, ".csv", sep = "")))
...
new_smooth_lagged_effects    = read.csv(file.path(regression_dir, paste("new_smooth_lagged_effects_", data_source, ".csv", sep = "")))
```
...
if (do_exclude_trials) {
  ...
  ts_params = merge(ts_params, subset(count_trials, select = -n_trials), all.x = T)
  ts_params = subset(ts_params, !few_trials)
  ts_params$few_trials = NULL
  
  # Exclude datasets with abnormal BICs
