...
ggplot(wide_dat, aes(a2.M, a2.K, color = hier)) +
  geom_point(alpha = .2) +
  labs(x = "alpha Maria", y = "alpha Klaus", color = "") +
  facet_grid(~ n_par)
...
K = reshape(subset(all_genrec, hier == "flat" & model == "K", select = c("SubjID", "n_par", "run", "BIC", "model")),
            idvar = c("SubjID", "run", "model"), timevar = "n_par", direction = "wide")
M = reshape(subset(all_genrec, hier == "flat" & model == "M", select = c("SubjID", "n_par", "run", "BIC", "model")),
            idvar = c("SubjID", "run", "model"), timevar = "n_par", direction = "wide")
rel_BIC = rbind(K, M)
...
best_model = subset(all_genrec, model == "M" & n_par == 4 & run != "Run 4" & hier == "flat")
...
ggplot(subset(params_regr_dat, odds < 5 & odds > -5), aes(w, odds, color = run)) +
  geom_point() +
