...
if (do_exclude_trials) {
  ...
  ts_params = subset(ts_params, BIC < (mean(ts_params$BIC) + 5 * sd(ts_params$BIC)) & !SubjID == 106)
}
...
summary(rel_BIC)
rel_BIC_molten = melt(subset(rel_BIC, select = -c(BIC.a1b1, BIC.hyb, BIC.l0, BIC.l1, BIC.mb, BIC.mf, BIC.nok, BIC.nop, BIC.nopk, BIC.b20)),
...
rel_AIC$nokp = with(rel_AIC, AIC.nopk - AIC.hyb)
...
summary(rel_AIC)
rel_AIC_molten = melt(subset(rel_AIC, select = -c(AIC.a1b1, AIC.hyb, AIC.l0, AIC.l1, AIC.mb, AIC.mf, AIC.nok, AIC.nop, AIC.nopk, AIC.b20)),
                      id.vars = c("SubjID", "session", "run", "training", "training_s1"))
...
binom.test(l1_better, l1_total)   # * (p = 0.001017)
...
# Add training column
selected_parameter_dat = subset(ts_params_molten, params == "l1" & parameter %in% c("a1", "a2", "b1", "b2", "l", "p", "k", "w"), select = c("SubjID", "run", "training", "training_s1", "parameter", "estimate"))
