...
* fit all models to the different training conditions
...
limits.key_perseverance_trials = 15      # Limit to exclude individual trials: In how many trials in a row may a subject press the same button without theses trials being excluded?
...
limits.exclude_subject = 0.75   # Limit to exclude the whole 2-step task from one subject: Did the subject loose more than 75% of the data during pre-processing?
```
...
count_too_fast_RTs = ddply(ts, c("SubjID", "session", "run"), summarize,
                           fast_trials = sum(RT1 < limits.trial_RTs, na.rm = T))
n_trials_fast = sum(ts$RT1 <= limits.trial_RTs, na.rm = T)   # Count how many are removed
...
cat("\nNumber of excluded blocks because of lacking choice1 variation (cut-off:", limits.stage1_choice_variation, "msec):\n")
...
```{r, echo = F, fig.cap = "Participants' absolute, raw probabilities."}
# Prepare data
...
if (ggsave_figures) {
  ...
  ggsave(plot = stay_switch_plot_s1, filename = paste(figure_dir, "stay_switch_plot_s1.png", sep = "/"), width = 5, height = 5)
}
```

```{r, echo = F, fig.cap = "Probabilities relative to participants' baseline (= mean) stay probabilities."}
...
```{r, echo = F, warning = F, fig.cap = "Changes over time: Within-participant differences between Run 1 and Runs 2 and 3."}


```

## Stats: Logistic regression (switch_stay ~ reward * transition)

```{r, echo = F, fig.cap = "Logistic regression: Stay ~ 1 + reward * transition"}
### Switch-stay stats
if (preprocess_data) {
  
  # Logistic regression separately for each group and run (Stay ~ 1 + reward * transition)
  switch_stay_stats_run_train = data.frame(predictor = NA, run = NA, training = NA, z = NA, p = NA)[0,]
  row = 1
  for (runi in unique(stay_dat$run)) {
    for (traini in unique(stay_dat$training_s1)) {
      model_dat = subset(ts, run == runi & training_s1 == traini, select = c("SubjID", "run", "training_s1", "stay_first_stage", "reward_prev_trial", "transi_prev_trial"))
      ...
      if (!empty(model_dat)) {
        ...
        switch_stay_stats_run_train[row:(row+3),1] = c("sta", "rew", "tra", "int")   # Which predictor?
        switch_stay_stats_run_train[row:(row+3),2:3] = c(rep(runi, 4), rep(traini, 4))   # Run and group
        switch_stay_stats_run_train[row:(row+3),4:5] = coefficients(summary(mod))[,3:4]   # z and p values
  ...
  switch_stay_stats_run = data.frame(predictor = NA, run = NA, chisq = NA, df = NA, p = NA)[0,]
  row = 1
  for (runi in unique(stay_dat$run)) {
    ...
    contrasts(model_dat$reward_prev_trial) = c(-1, 1)
    contrasts(model_dat$transi_prev_trial) = c(-1, 1)
    contrasts(model_dat$training_s1) = cbind(c(-2, 1, 1), c(0,-1, 1))
    
    if (!empty(model_dat)) {
      mod = glmer(stay_first_stage ~ reward_prev_trial * transi_prev_trial * training_s1 + (reward_prev_trial * transi_prev_trial | SubjID),
            family = binomial, control = glmerControl(optimizer = "bobyqa"),
            data = model_dat)
      aov = Anova(mod, type = 3)
      
      switch_stay_stats_run[row:(row+7),1] = c("sta", "rew", "tra", "gro", "int", "rewgro", "tragro", "intgro")   # Which predictor?
      switch_stay_stats_run[row:(row+7),2] = rep(runi, 4)   # Run and group
      switch_stay_stats_run[row:(row+7),3:5] = aov   # z and p values
  ...
  mod = glmer(stay_first_stage ~ reward_prev_trial + transi_prev_trial + reward_prev_trial:transi_prev_trial + reward_prev_trial:training + reward_prev_trial:transi_prev_trial:training + reward_prev_trial:run + reward_prev_trial:transi_prev_trial:run +
                (reward_prev_trial + transi_prev_trial + reward_prev_trial:transi_prev_trial + reward_prev_trial:run + reward_prev_trial:transi_prev_trial:run | SubjID),
      family = binomial, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6)),
      data = subset(ts, !is.na(reward_prev_trial) & run != "Run 4"))
  switch_stay = coefficients(summary(mod))
  
  write.csv(switch_stay_stats_run_train, file.path(data_dir, paste("switch_stay_stats_run_train", data_source, ".csv", sep = "")), row.names = F)
  write.csv(switch_stay_stats_run, file.path(data_dir, paste("switch_stay_stats_run", data_source, ".csv", sep = "")), row.names = F)
  write.csv(switch_stay, file.path(data_dir, paste("switch_stay", data_source, ".csv", sep = "")), row.names = T)
  
}

# Read data from disc
switch_stay_stats_run_train = read.csv(file.path(data_dir, paste("switch_stay_stats_run_train", data_source, ".csv", sep = "")))
switch_stay_stats_run = read.csv(file.path(data_dir, paste("switch_stay_stats", data_source, ".csv", sep = "")))
switch_stay = read.csv(file.path(data_dir, paste("switch_stay", data_source, ".csv", sep = "")))

switch_stay_stats_run$z = as.numeric(switch_stay_stats_run$z)
switch_stay_stats_run$predictor = factor(switch_stay_stats_run$predictor, levels = c("sta", "rew", "tra", "int"))
switch_stay_stats_run$sig = "ns"
switch_stay_stats_run$sig[switch_stay_stats_run$p <= 0.05] = "sig"
switch_stay_stats_run[is.na(switch_stay_stats_run$predictor),]$predictor

switch_stay_stats_run_train$z = as.numeric(switch_stay_stats_run_train$z)
switch_stay_stats_run_train$predictor = factor(switch_stay_stats_run_train$predictor, levels = c("sta", "rew", "tra", "int"))
switch_stay_stats_run_train$sig = "ns"
switch_stay_stats_run_train$sig[switch_stay_stats_run_train$p <= 0.05] = "sig"
switch_stay_stats_run_train[is.na(switch_stay_stats_run_train$predictor),]$predictor

## Plot / show
### Separate for runs and groups
gg_overall_stats = ggplot(subset(switch_stay_stats_run_train, run != "Run 4"), aes(predictor, z, shape = sig)) +
...
gg_overall_stats_run = ggplot(subset(switch_stay_stats_run, run != "Run 4"), aes(predictor, z, shape = sig)) +
  geom_point(color = "darkblue") +
  scale_shape_manual(values = c(1, 16)) +
  geom_text(aes(label = ifelse(as.numeric(p) < 0.1, round(as.numeric(p), 3), "")), hjust = 0, vjust = 0, size = 3) +
  labs(x = "", y = "z") +
...
# switch_stay_stats_run

### Grand model
switch_stay

if (ggsave_figures) {
  ggsave(plot = gg_overall_stats, filename = paste(figure_dir, "stay_switch_plot_stats.png", sep = "/"), width = 5, height = 5)
}
```

### Separate logistic regression for each participant (switch_stay ~ stay + reward + transition + interaction + correct)
...
### Normalize parameters (part accidentally deleted - necssary for old data)
...
# Plot average BICs
BIC = ggplot(subset(ts_params_molten, parameter == "a1"), aes(params, BIC, group = 1, color = factor(num_par))) +
...
# Prepare relative BICs
rel_BIC = reshape(subset(ts_params_molten, parameter == "a1", select = -c(num_par, estimate, parameter)),
                  idvar = c("SubjID", "session", "run", "training", "training_s1"), timevar = "params", direction = "wide")
rel_BIC$nop = with(rel_BIC, BIC.hyb - BIC.nop)
rel_BIC$nok = with(rel_BIC, BIC.hyb - BIC.nok)
rel_BIC$mb = with(rel_BIC, BIC.hyb - BIC.mb)
rel_BIC$mf = with(rel_BIC, BIC.hyb - BIC.mf)
rel_BIC$a1b1 = with(rel_BIC, BIC.hyb - BIC.a1b1)
rel_BIC$l0 = with(rel_BIC, BIC.hyb - BIC.l0)
rel_BIC$l1 = with(rel_BIC, BIC.hyb - BIC.l1)
rel_BIC$nok_mb = with(rel_BIC, BIC.hyb - BIC.nok_mb)
rel_BIC$nok_mf = with(rel_BIC, BIC.hyb - BIC.nok_mf)
rel_BIC$nok_l0 = with(rel_BIC, BIC.hyb - BIC.nok_l0)
rel_BIC$nok_l1 = with(rel_BIC, BIC.hyb - BIC.nok_l1)
rel_BIC$a1b1_nok = with(rel_BIC, BIC.hyb - BIC.a1b1_nok)
summary(rel_BIC)
rel_BIC_molten = melt(subset(rel_BIC, select = -c(BIC.a1b1_nok, BIC.a1b1, BIC.hyb, BIC.l0, BIC.l1, BIC.mb, BIC.mf, BIC.nok_l0, BIC.nok_l1, BIC.nok_mb, BIC.nok_mf, BIC.nok, BIC.nop, BIC.nopk)),
                      id.vars = c("SubjID", "session", "run", "training", "training_s1"))

# Plot relative BICs
gg_rel_BIC = ggplot(subset(rel_BIC_molten, run != "Run 4"), aes(variable, -value)) +
  geom_point(position = "jitter", alpha = 0.2) +
  stat_summary(fun.data = mean_cl_normal, geom = "pointrange", color = "red") +
  labs(x = "", y = "model BIC - hyb BIC")
gg_rel_BIC

gg_rel_BIC_by_group = gg_rel_BIC +
  facet_grid(run ~ training_s1)
gg_rel_BIC_by_group

if (ggsave_figures) {
  ggsave(file.path(figure_dir, "gg_rel_BIC.png"), gg_rel_BIC)
  ggsave(file.path(figure_dir, "gg_rel_BIC_by_group.png"), gg_rel_BIC_by_group)
}

# Stats
nok_total = sum(!is.na(rel_BIC$nok))
nok_better = sum(rel_BIC$nok > 0, na.rm = T)
binom.test(nok_better, nok_total)   # ***
nop_total = sum(!is.na(rel_BIC$nop))
nop_better = sum(rel_BIC$nop > 0, na.rm = T)
binom.test(nop_better, nop_total)   # .
...
nok_mb_total = sum(!is.na(rel_BIC$nok_mb))
nok_mb_better = sum(rel_BIC$nok_mb > 0, na.rm = T)
binom.test(nok_mb_better, nok_mb_total)   # ***
nok_mf_total = sum(!is.na(rel_BIC$nok_mf))
nok_mf_better = sum(rel_BIC$nok_mf > 0, na.rm = T)
binom.test(nok_mf_better, nok_mf_total)   # ***
nok_l1_total = sum(!is.na(rel_BIC$nok_l1))
nok_l1_better = sum(rel_BIC$nok_l1 > 0, na.rm = T)
binom.test(nok_l1_better, nok_l1_total)   # ***
...
hyb_nok_mod = lmer(BIC ~ params + (params | SubjID), data = lmer_dat1, REML = F)
summary(hyb_nok_mod)
...
for (parami in unique(selected_parameter_dat$parameter)) {
  for (subj in unique(selected_parameter_dat$SubjID)) {
    ...
    if (nrow(dat) == 4) {
      ...
      if (length(d13) > 0) {diff_dat[row+3,] = c(subj, 13, parami, d13)}
      if (length(d14) > 0) {diff_dat[row+4,] = c(subj, 14, parami, d14)}
