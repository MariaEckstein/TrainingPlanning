...
```{r Set script parameters, results = F, warning = F}
...
```
```{r Load libraries, echo = F, results = F, warning = F, message = F}
...
```
```{r Create separate logistic regression for each participant, echo = F, warning = F, message = F, fig.cap = "Log odds"}
...
```
```{r Plot regression models results (odds ratios for main effects of transition and reward and interaction, echo = F}
...
```
```{r Plot model fits for the models with differnt numbers of predictors, echo = F}
...
if (do_exclude_trials) {
  ...
  # Exclude datasets when training performance was bad
