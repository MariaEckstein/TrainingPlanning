...
num_par = 4   # number of parameters for Klaus' models
  ...
  for (fileid in (1:dim(mat_file)[3])) {
    ...
    for (hier in 1:2) {
      ...
      for (run in 1:4) {
        ...
        subj_par4 = c(subjID, run, par4[1], NA, par4[2], NA, 0, par4[4], par4[3], NA, NA, BIC, AIC, hier)
        # Add to ts_params
        ts_params = rbind(ts_params, subj_par4)
      }
    }
  }
  colnames(ts_params) = c(ts_colnames, "params")
  ts_params$num_par = num_par
  ts_params$params = paste("a1b1_l0k0", ifelse(ts_params$params == 1, "flat", "hier"), sep = "_")
  ts_params$session = paste("Session", ifelse(ts_params$run %in% 1:2, 1, 2))
  write.csv(ts_params, file.path(model_dir, paste("KlausModelingResults", num_par, ".csv", sep = "")), row.names = F)
}
ts_params = read.csv(file.path(model_dir, paste("KlausModelingResults", num_par, ".csv", sep = "")))
...
gg_w_flat_hier = ggplot(w_molten, aes(w.a1b1_l0k0_flat, w.a1b1_l0k0_hier)) +
  geom_point() +
  geom_abline(slope = 1)
gg_w_flat_hier

gg_w_k_m = ggplot(w_molten, aes(w.a1b1_l0k0_flat, w.l0)) +
  geom_point() +
  geom_abline(slope = 1)
gg_w_k_m

gg_w_hyb_l0 = ggplot(w_molten, aes(w.hyb, w.l0)) +
  geom_point() +
  geom_abline(slope = 1)
gg_w_hyb_l0
...
gg_p_flat_hier = ggplot(p_molten, aes(p.a1b1_l0k0_flat, p.a1b1_l0k0_hier)) +
  geom_point() +
  geom_abline(slope = 1)
gg_p_flat_hier

gg_p_k_m = ggplot(p_molten, aes(p.a1b1_l0k0_flat, p.l0)) +
  geom_point() +
  geom_abline(slope = 1/13, intercept = 0.5)
gg_p_k_m

gg_p_hyb_l0 = ggplot(p_molten, aes(p.hyb, p.l0)) +
  geom_point() +
  geom_abline(slope = 1) +
...
gg_a_flat_hier = ggplot(a_molten, aes(a1.a1b1_l0k0_flat, a1.a1b1_l0k0_hier)) +
  geom_point() +
  geom_abline(slope = 1) +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1))
gg_a_flat_hier

gg_a_k_m = ggplot(a_molten, aes(a1.a1b1_l0k0_flat, a1.l0)) +
  geom_point() +
  geom_abline(slope = 1) +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1))
gg_a_k_m

gg_a_hyb_l0 = ggplot(a_molten, aes(a1.hyb, a1.l0)) +
  geom_point() +
  geom_abline(slope = 1) +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1))
gg_a_hyb_l0

b_molten = reshape(subset(ts_params, select = c("SubjID", "run", "b1", "params")),
                   idvar = c("SubjID", "run"), timevar = "params", direction = "wide")

gg_b_flat_hier = ggplot(b_molten, aes(b1.a1b1_l0k0_flat, b1.a1b1_l0k0_hier)) +
  geom_point() +
  geom_abline(slope = 1)
gg_b_flat_hier

gg_b_k_m = ggplot(b_molten, aes(b1.a1b1_l0k0_flat, b1.l0)) +
  geom_point() +
  geom_abline(slope = 1/40)
gg_b_k_m
```
...
ggplot(subset(params_regr_dat, params %in% c("a1b1_l0k0_flat", "a1b1_l0k0_hier", "l0") & odds < 5 & odds > -5), aes(w, odds)) +
  geom_point() +
...
rel_BIC$k_flat = with(rel_BIC, BIC.a1b1_l0k0_flat - BIC.hyb)
rel_BIC$k_hier = with(rel_BIC, BIC.a1b1_l0k0_hier - BIC.hyb)
summary(rel_BIC)
rel_BIC_molten = melt(subset(rel_BIC, select = -c(BIC.a1b1, BIC.hyb, BIC.l0, BIC.l1, BIC.mb, BIC.mf, BIC.nok, BIC.nop, BIC.b20, BIC.a1b1_l0k0_flat, BIC.a1b1_l0k0_hier)),
...
rel_AIC$k_flat = with(rel_AIC, AIC.a1b1_l0k0_flat - AIC.hyb)
rel_AIC$k_hier = with(rel_AIC, AIC.a1b1_l0k0_hier - AIC.hyb)
summary(rel_AIC)
rel_AIC_molten = melt(subset(rel_AIC, select = -c(AIC.a1b1, AIC.hyb, AIC.l0, AIC.l1, AIC.mb, AIC.mf, AIC.nok, AIC.nop, AIC.b20, AIC.a1b1_l0k0_flat, AIC.a1b1_l0k0_hier)),
                      id.vars = c("SubjID", "session", "run", "training", "training_s1"))
...
selected_parameter_dat = subset(ts_params_molten, params == "a1b1_l0k0_hier" & parameter %in% c("a1", "a2", "b1", "b2", "p", "k", "w"), select = c("SubjID", "run", "training", "training_s1", "parameter", "estimate"))   # params == "l1"
