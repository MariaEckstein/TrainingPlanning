...
do_exclude_bad_training_blocks = T      # Exclude 2-step blocks that come from sessions with really bad training task performance?
...
ts = merge(ts, count_trials, all = T)
...
logist_regr_data_AICs_long = merge(logist_regr_data_AICs_long_Chi, logist_regr_data_AICs_long_p, by = c("run", "SubjID", "training", "effect"), all = T)
...
if (do_exclude_trials) {
  ...
  ts_params = merge(ts_params, subset(count_trials, select = -n_trials), all = T)
