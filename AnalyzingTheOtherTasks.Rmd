...
finalACC = ddply(subset(cat_dat, trial >= 90), .(SubjID, run, task, training, training_s1), summarize,
                 finACC = mean(ACC, na.rm = T))
sum_cat_dat = merge(sum_cat_dat, finalACC)
ii_trials = subset(sum_cat_dat, task == "ii")$finACC
ii_median = median(ii_trials, na.rm = T)
sum_cat_dat$median[sum_cat_dat$task == "ii"] = ifelse(ii_trials > ii_median, "above", "below")
rb_trials = subset(sum_cat_dat, task == "rb")$finACC
rb_median = median(rb_trials, na.rm = T)
sum_cat_dat$median[sum_cat_dat$task == "rb"] = ifelse(rb_trials > rb_median, "above", "below")
sum_cat_dat$median = factor(sum_cat_dat$median)
summary(sum_cat_dat)
...
bad_perf = merge(bad_perf,
                 subset(sum_cat_dat, task == "ii", select = c("SubjID", "run", "ACC", "finACC", "median")),
                 all = T)
colnames(bad_perf)[colnames(bad_perf) %in% c("ACC", "finACC", "median")] = c("catii_ACC", "catii_finACC", "catii_median")

bad_perf = merge(bad_perf,
                 subset(sum_cat_dat, task == "rb", select = c("SubjID", "run", "ACC", "finACC", "median")),
                 all = T)
colnames(bad_perf)[colnames(bad_perf) %in% c("ACC", "finACC", "median")] = c("catrb_ACC", "catrb_finACC", "catrb_median")
...
sum_ori_dat = ddply(ori_dat, .(SubjID, run, training, training_s1), summarize,
                    ACC = mean(ACC, na.rm = T),
                    RT = median(RT, na.rm = T))
summary(sum_ori_dat)

gg_ori_over_trials3 = ggplot(sum_ori_dat, aes(run, 100 * ACC, group = 1)) +
  stat_summary(fun.y = mean, geom = "point", color = "black") +
  geom_point(alpha = .5, position = "jitter") +
  geom_hline(yintercept = 50) +
...
bad_perf = merge(bad_perf,
                 subset(sum_ori_dat, select = c("SubjID", "run", "ACC")),
                 all = T)
...
sum_num_dat = ddply(num_dat, .(SubjID, run, training, training_s1), summarize,
                    ACC = mean(ACC, na.rm = T))
summary(sum_num_dat)

gg_num_over_trials3 = ggplot(sum_num_dat, aes(run, 100 * ACC, group = 1)) +
  stat_summary(fun.y = mean, geom = "point", color = "black") +
  geom_point(alpha = .5, position = "jitter")
gg_num_over_trials3
```
...
bad_perf = merge(bad_perf,
                 subset(sum_num_dat, select = c("SubjID", "run", "ACC")),
                 all = T)
...
# co
gg_mbtasks = ggplot(bad_perf, aes(ori_ACC, num_ACC, color = training, group = run)) +
  geom_point(alpha = .5) +
...
# control
gg_cotasks = ggplot(subset(bad_perf, !(ori_bad | num_bad)), aes(ori_ACC, num_ACC, color = training, group = run)) +
  geom_point(alpha = .5) +
  labs(x = "Orientation discrimination (% correct)", y = "Number comparison (% correct)") +
  geom_smooth(method = lm, se = F, color = "black") +
  facet_wrap(~ run)
