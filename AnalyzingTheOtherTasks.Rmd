...
```{r Theoretical reward probabilities, echo = F, warning = F, message = F}
...
```
```{r Read in data and summarize by trials, echo = F, warning = F, message = F}
...
head(food_dat)
```
```{r Performance over time, echo = F, message = F, warning = F}
...
```
```{r Performance by subject, echo = F, message = F, warning = F}
...
```{r Read in data, echo = F, message = F, warning = F}
